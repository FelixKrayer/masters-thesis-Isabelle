...
\section{Linearizing MRBNFs}
  ...
  We formalize the idea of distinctness of components as \textit{non-repetitiveness} on the linearized variables. In our notation we use $L$ as the set of indices of live variables we linearize on, i.e., $L \subseteq \{1 \dots l\}$. Furthermore we define a combinator $R^l \bowtie^L Q^l = [(\texttt{if}\; i \in L\;\texttt{then}\; Q_i\; \texttt{else}\; R_i)\; \texttt{for}\; 1 \leq i \leq l]$ that selects the second argument if $i$ is a variable to be linearized on and the first one, if not.
  ...
  \label{subsec:nonrep}
    ...
    \begin{align*}
      \eqshape_F^L\; x\; y &= \rel_F\; ((=)^l \bowtie^L (\top)^l)\; x\; y\\
      \nonrep_F^L\; x &= \forall y.\; \eqshape_F^L\; x\; y \Longrightarrow (\exists f^l.\; y = \map_f\; \langle(\id^l \bowtie^L f^l)\; \id^\fr\; \id^b\rangle\; x)
    \end{align*}
  ...
  \label{subsec:conditions}
    ...
    \begin{align*}
      ...
      &\smash{\map_F\; \langle\fst^l\; \id^\fr\; \id^b\rangle\; z = x \land \map_F\; \langle\snd^l\; \id^\fr\; \id^b\rangle\; z = y}
    \end{align*}
  ...
  \subsection{Intermediate lemmas}
    ...
    \label{par:rel_exchange}
      ...
      For our purpose of linearization, we are specifically interested in the case, where for all live variables that we linearize on the relation from $R^l$ is chosen and for all others the relation from $Q^l$ relation, i.e., the result of the combinator $\bowtie^L$. This results in the following lemma for a \ac{MRBNF} $F$:
      \begin{equation*}
        \rel_F\; R^l\; x\; y\; \land\; \rel_F\; Q^l\; x\; y \Longrightarrow \rel_F\; (Q^l \bowtie^L R^l)\; x\; y \tag{\textsc{rel\_exchange}}\label{eq:rel_exchange}
      \end{equation*} 
      In the specific case, that the \ac{MRBNF} is linearized on \textit{all} of it's live variables, the combination that is chosen is exactly $R^l$. Then the lemma becomes trivial, since its goal is equal to it's first assumption in this case.
    ...
    \subsubsection{map peresrving non-repetitiveness}
      \begin{align*}
        ...
        &\textsf{bijective}\; f^l\; \land\; \nonrep_F^L\; x\; \Longrightarrow \nonrep_F^L\; (\map_F\; \langle(g^l \bowtie_L f^l)\; v^\fr\; u^b\rangle\; x)
      \end{align*} 
    ...
    \subsubsection{map reflecting non-repetitiveness}
      ...
      \begin{equation*}
        \nonrep_F^L\; (\map_F\; \langle(f^l \bowtie_L \id^l)\; \id^\fr\; \id^b\rangle\; x)\Longrightarrow \nonrep_F^L\; x \tag{\textsc{nonrep\_map\_rev}}\label{eq:nrp_map_rev}
      \end{equation*} 
    ...
    \subsection{Proving the MRBNF axioms}
      ...
      \begin{align*}
        ...
        \equiv\; &\nonrep_F^L\; (\map_{F}\; f^{v\!s}\; (\rep_{F'}\; x)) \Longrightarrow\set_{F,i}\; (\map_{F}\; f^{v\!s}\; (\rep_{F'}\; x))& \text{$\abs$ inverse}\\
        \equiv\; &\nonrep_F^L\; (\rep_{F'}\; x) \Longrightarrow\set_{F,i}\; (\map_{F}\; f^{v\!s}\; (\rep_{F'}\; x))& \text{\ref{eq:nrp_map}}\\
      ...
      We don't show a full proof of this property here, but investigate an interesting step. In the proof we reach a state, where we need to show that $\nonrep_F^L\; (\map_F\; \langle \fst^l\; \id^\fr\; \id^b\rangle\; z) \Longrightarrow \nonrep_F^L\; (\map_F\; \langle (\id^l \bowtie_L \fst^l)\; \id^\fr\; \id^b\rangle\; z)$. To give an intuition for why this is necessary, we obtain the left side of the implication from the \ref{eq:in_rel} axiom of $F$ and need to show the right side to eliminate a composition $\abs_{F'} \circ \rep_{F'}$ in the goal state. 

      \begin{align*}
        &\nonrep_F^L\; (\map_F\; \langle \fst^l\; \id^\fr\; \id^b\rangle\; z) \Longrightarrow\\
        &\nonrep_F^L\; (\map_F\; \langle (\fst^l \bowtie_L \id^l)\; \id^\fr\; \id^b\rangle\; (\map_F\; \langle (\id^l \bowtie_L \fst^l)\; \id^\fr\; \id^b\rangle\; z)) \Longrightarrow\\
        &\nonrep_F^L\; (\map_F\; \langle (\id^l \bowtie_L \fst^l)\; \id^\fr\; \id^b\rangle\; z)\\
      \end{align*}
      ...
      %Another interesting step is the conversion from $\mrrel_{F'}$ to $\mrrel_F$. While $\mrrel_{F'}$ takes functions for the linearized type variables - that turned to bounds, the relator of original \ac{MRBNF} $F$ takes relations for these. By explicitly specifying 
      % TODO: not done! needs l', b' and L_b. Also big issue with this in general...
      %$\rel_F \; (R^l \bowtie_L (\Grp f)^l)\; (\map_F \langle \id^l\; v^\fr\; u^b \rangle x)\; y$
      %$\rel_F \; (R^l \bowtie_L (=)^l)\; (\map_F \langle (\id^l \bowtie_L f^l)\; v^\fr\; u^b \rangle x)\; y$
      %$\rel_{F'} \; R^{l'}\; (\map_{F'} \langle (\id^l \bowtie_L f^l)\; v^\fr\; u^b \rangle x)\; y$
  ...
  \section{Implementing the \textbf{linearize\_mrbnf} command}
    ...
    \begin{align*}
      \forall x\; y.\; (&\map_F\; \langle\fst^l\; \id^\fr\; \id^b\rangle\; x = \map_F\; \langle\fst^l\; \id^\fr\; \id^b\rangle\; y\; \land\\
      &\map_F\; \langle\snd^l\; \id^\fr\; \id^b\rangle\; x = \map_F\; \langle\snd^l\; \id^\fr\; \id^b\rangle\; y)\Longrightarrow\\ 
      &x = y
    \end{align*}
