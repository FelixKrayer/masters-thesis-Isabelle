...
\section{Linearizing MRBNFs}
  ...
  \label{subsec:nonrep}
    ...
    \begin{align*}
      ...
      \nonrep_F^L\; x &= \forall y.\; \eqshape_F^L\; x\; y \Longrightarrow (\exists f^l.\; y = \map_f\; (\id^l \bowtie^L f^l)\; \id^\fr\; \id^b\; x)
    \end{align*}
  ...
  \subsection{Intermediate lemmas}
    ...
    \subsubsection{map reflecting non-repetitiveness}
      ...
      \begin{equation*}
        \nonrep_F^L\; (\map_F\; (f^l \bowtie_L \id^l)\; \id^\fr\; \id^b\; x)\Longrightarrow \nonrep_F^L\; x \tag{\textsc{nonrep\_map\_rev}}\label{eq:nrp_map_rev}
      \end{equation*} 
    ...
    \subsection{Defining the subtype and its constants}
      ...
      \begin{align*}
        ...
        \map_{F'}\;  f^l\;u^\fr\;v^b  &= \abs_{F'} \circ (\map_{F}  (f^l \bowtie_F (\asBij f^l))\; v^\fr\; u^b ) \circ \rep_{F'}\\
    ...
    \subsection{Proving the MRBNF axioms}
      ...
      We don't show a full proof of this property here, but investigate an interesting step. In the proof we reach a state, where we need to show that $\nonrep_F^L\; (\map_F\;  \fst^l\; \id^\fr\; \id^b\; z) \Longrightarrow \nonrep_F^L\; (\map_F\;  (\id^l \bowtie_L \fst^l)\; \id^\fr\; \id^b\; z)$. To give an intuition for why this is necessary, we obtain the left side of the implication from the \ref{eq:in_rel} axiom of $F$ and need to show the right side to eliminate a composition $\abs_{F'} \circ \rep_{F'}$ in the goal state. 

      \begin{align*}
        &\nonrep_F^L\; (\map_F\;  \fst^l\; \id^\fr\; \id^b\; z) \Longrightarrow\\
        &\nonrep_F^L\; (\map_F\;  (\fst^l \bowtie_L \id^l)\; \id^\fr\; \id^b\; (\map_F\;  (\id^l \bowtie_L \fst^l)\; \id^\fr\; \id^b\; z)) \Longrightarrow\\
        &\nonrep_F^L\; (\map_F\;  (\id^l \bowtie_L \fst^l)\; \id^\fr\; \id^b\; z)\\
      \end{align*}
      ...
      %$\rel_F \; (R^l \bowtie_L (\Grp f)^l)\; (\map_F  \id^l\; v^\fr\; u^b  x)\; y$
      %$\rel_F \; (R^l \bowtie_L (=)^l)\; (\map_F  (\id^l \bowtie_L f^l)\; v^\fr\; u^b  x)\; y$
      %$\rel_{F'} \; R^{l'}\; (\map_{F'}  (\id^l \bowtie_L f^l)\; v^\fr\; u^b  x)\; y$
