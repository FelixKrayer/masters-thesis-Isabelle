...
\section{Linearization of MRBNFs (In theory)}
  ...
  \section{Linearization of MRBNFs (In Isabelle)}
    ...
    \begin{equation*}
      \textbf{linearize\_mrbnf}\; (\text{keys:}\; \kappa :: \text{var},\; \text{vals:}\; \upsilon)\; \text{alist} = (\kappa :: \text{var} \times \upsilon)\; \text{list}\; \textbf{on}\; \kappa
    \end{equation*}
    Since for \type{$(\kappa \times \upsilon)$ list} both type variables are live and we only linearize on $\kappa$, it is necessary to prove strong pullback preservation for this \ac{MRBNF}. 
