...
% "atom" or "component" - refers to instances of the type variables ($\alpha$, $\beta$, \dots) of a type constructor $F$ that "make up" an $F$ element
  ...
  \section{Map-Restricted Bounded Natural Functors (MRBNFs)}
    ...
    To relate $F$ elements that are not equal in all frees and bounds, we introduce a new map-restricted relator $\mrrel_F$. It takes a function for each free and bound - with the appropriate restrictions to small-support and bijectivity - in addition to the relations for the lives. It then uses the graphs $\Grp$ of these functions as relations for the respective free or bound variable. Transferring the ideas of bijectivity ans small-support to these graph relations, it means, that 
    
    The new arguments are placed in front of the relations for the live variables. It is then defined in terms of the relator as shown below. Note, that relating two elements with the graph of a function $v$ is equivalent to mapping $v$ over the first element and relating that to the second one by equality.
    \[
      \mrrel_F\; \langle u^{b}\; v^{\fr}\rangle\; R^l\; x\; y = \rel_F\; R^l\; (\map_F\; \langle\id^{l}\; v^{\fr}\; u^b\rangle\; x)\; y
    \]
    Here we write $f^l$ for the functions or relations of the live variables $f_1 \dots f_l$ and analogously $v^\fr$ and $u^b$ for frees and bounds. Furthermore, we write the arguments of the map function as $\langle f^{l}\; v^{\fr}\; u^b\rangle$. The argument order of the mapper might be different, as the lives, bounds and frees do not have to be separated, but can be interlaced in some order. For example for the type $(\alpha,\; \beta,\; \gamma)\; F$ where $\alpha$ and $\gamma$ are free, while $\beta$ is bound, the mapper has the following type:
    \[
    ...
    \]
    Thus, in our notation $\map_F\; \langle f^{l}\; v^{\fr}\; u^b\rangle$ we assume the $\langle\dots\rangle$ to interlace the arguments correctly to $\map_F\; v_1\; u_1\; v_2$. Analogously for the $\mrrel_F$, $\langle u^{b}\; v^{\fr}\rangle$ interlaces the functions according to the order of bounds and frees.
    ...
    \subsection{MRBNF axioms}
      ...
      \begin{align*}
        \mrrel_F\; \langle u^{b}\; v^{\fr}\rangle\; R^l\; x\; y =\; &\exists z.\; (\forall i.\; \set_{F,i}\; z \subseteq \{(a, b).\; R_i\; a\; b \}) \land \\
        &\smash{\map_F\; \langle\fst^{l}\; \id^{\fr}\; \id^b\rangle\; z = x \land \map_F\; \langle\snd^{l}\; v^{\fr}\; u^b\rangle\; z = y}
      \end{align*}
