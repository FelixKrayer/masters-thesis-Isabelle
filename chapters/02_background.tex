...
% "atom" or "component" - refers to instances of the type variables ($\alpha$, $\beta$, \dots) of a type constructor $F$ that "make up" an $F$ element
  ...
  \section{Bounded Natural Functors (BNFs)}
    ...
    \subsection{BNF-axioms}
      ...
      \begin{figure}
        \centering
        \begin{align*}
          \vphantom{\overline{f}^l}&\map_F\; \overline{\id}^l\; x = x \tag{\textsc{map\_id}}\label{eq:map_id}\\
          \vphantom{\overline{f}^l}&\map_F\; \overline{g}^l\; (\map_F\; \overline{f}^l\; x) =
            \map_F\; \overline{(g \circ f)}^l\; x \tag{\textsc{map\_comp}}\label{eq:map_comp}\\
          \vphantom{\overline{f}^l}&(\forall i.\; \forall z \in \set_{F,i}\; x.\; f_i\; z = g_i\; z) \Longrightarrow 
            \map_F\; \overline{f}^l\; x = \map_F\; \overline{g}^l\; x \tag{\textsc{map\_cong}}\label{eq:map_cong}\\
          \vphantom{\overline{f}^l}&\forall i.\; \set_{F,i} (\map_F\; \overline{f}^l\; x) = f_i\; \grave{\phantom{\_}}\; \set_{F,i}\; x \tag{\textsc{set\_map}}\label{eq:set_map}\\
          \vphantom{\overline{f}^l}&\textsf{infinite}\; \bd_F \land \textsf{regular}\; \bd_F \land 
            \textsf{cardinal\_order}\; \bd_F \tag{\textsc{bd}}\label{eq:bd}\\
          \vphantom{\overline{f}^l}&\forall i.\; | \set_{F,i}\; x | <_o \bd_F \tag{\textsc{set\_bd}}\label{eq:set_bd}\\
          \vphantom{\overline{f}^l}&\rel_F\; \overline{R}^l\; \bullet\; \rel_F\; \overline{Q}^l\; = 
            \rel_F\; \overline{(R\; \bullet\; Q)}^l \tag{\textsc{rel\_compp}}\label{eq:rel_compp}\\
          \vphantom{\overline{f}^l}&\rel_F\; \overline{R}^l\; x\; y = \tag{\textsc{in\_rel}}\label{eq:in_rel}\\
          &\smash{\exists z.\; (\forall i.\; \set_{F,i}\; z \subseteq \{(a, b).\; R_i\; a\; b \}) \land 
              \map_F\; \overline{\fst}^l\; z = x \land \map_F\; \overline{\snd}^l\; z = y}
        \end{align*}
        \newline
        \footnotesize
      ...
      Distributivity of the relator is formulated in \ref{eq:in_rel}. We note here, that for showing that a type constructor is a \ac{BNF}, it is only necessary to prove the inclusion $(\rel_F\; \overline{R}^l\; \bullet\; \rel_F\; \overline{Q}^l)\; x\; y \Rightarrow \rel_F\; \overline{(R\; \bullet\; Q)}^l\; x\; y $. The other direction follows automatically from this and the next axiom, weak pullback preservation.
  ...
  \section{Map-Restricted Bounded Natural Functors (MRBNFs)}
    ...
    \[
      \mrrel_F\; \langle\overline{u}^{b}\; \overline{v}^{\fr}\rangle\; \overline{R}^l\; x\; y = \rel_F\; \overline{R}^l\; x\; (\map_F\; \langle\overline{\id}^{l}\; \overline{v}^{\fr}\; \overline{u}^b\rangle\; y)
    \]
    ...
    \]
    Thus, in our notation $\map_F\; \langle\overline{f}^{l}\; \overline{v}^{\fr}\; \overline{u}^b\rangle$ we assume the $\langle\dots\rangle$ to interlace the arguments correctly to $\map_F\; v_1\; u_1\; v_2$. Analogously for the $\mrrel_F$, $\langle\overline{u}^{b}\; \overline{v}^{\fr}\rangle$ interlaces the functions according to the order of bounds and frees.
    ...
    \subsection{MRBNF axioms}
      ...
      For the \ref{eq:map_comp}, \ref{eq:map_cong} and \ref{eq:set_map} axioms, we add the assumptions that the functions that correspond to frees and bounds are small-support functions and that the ones corresponding to bounds are additionally bijections. 

      Furthermore, while \ref{eq:rel_compp} stays unchanged, using the original relator, \ref{eq:in_rel} is changed to be defined in terms of the map-restricted relator.
      % TODO copy changes
