...
%    WIP: Considering a polymorphic type that is meant to represent simple $\lambda$-terms, where $\alpha$ is the space of variable names. If we want to substitute a free variable $x$ in a term $T$ by a term $N$, we may run into the following problem: If $T = \lambda y. T'$, we need to ensure that there are no name clashes with $y$ in the new term $N$ before we substitute $x$ by $N$ in $T'$. This is done by choosing a fresh $y'$ and renaming $y$ to $y'$ in $T'$. 
  ...
  \section{Map-Restricted Bounded Natural Functors (MRBNFs)}
    ...
    \label{subsec:binders}
      ...
      \end{align*}
      The core difference we notice compared to to a regular datatype definition, are the names given to certain components (e.g., \texttt{x}::$\alpha$) and the usage of these names to declare bindings (e.g., \textbf{binds} \texttt{x} \textbf{in} \texttt{t}). For the abstraction case Abs, this declaration means that for a term $\text{Abs}\; x\; t$, the variable $x$ is considered bound in $t$. 
      
      In general, any name occurring between \textbf{binds} and \textbf{in} corresponds to a type variable that is to be bound. In contrast to that, a name that occurs after the \textbf{in} corresponds to a recursive occurrence of the datatype in which the variables are being bound. Most importantly, the \textbf{binder\_datatype} command allows us to use a \ac{MRBNF} with bound variables in the definition of a datatype, if the bound variables correspond to type variables that are to be bound.
