...
% "atom" or "component" - refers to instances of the type variables ($\alpha$, $\beta$, \dots) of a type constructor $F$ that "make up" an $F$ element
  ...
  \section{Bounded Natural Functors (BNFs)}
    ...
    \subsection{BNF constants}
      ...
      \subsubsection{Bound and boundedness}
        ...
        The bound is used in the \autoref{eq:set_bd} axiom to ensure that the sets obtained by the setters are bounded. This ensures that the branching of a recursively defined datatype is also bounded and thus the resulting type $F$ as well.
      ...
      \subsubsection{Relator and shapes}
        ...
        \begin{align*}
          ...
          &\textsf{rel}_\textsf{prod}\; R\; Q\; p_1\; p_2 :=\; R\; (fst\; p_1)\; (fst\; p_2)\land Q\; (snd\; p_1)\; (snd\; p_2)
        \end{align*}
    ...
    \subsection{BNF-axioms}
      We formalize the \ac{BNF}-axioms in \autoref{fig:bnf_axioms} where we use the notation $\overline{f}^l = f_1 \dots f_l$ for the arguments of the mapper and the relator
      \begin{figure}
        \centering
        \begin{align*}
          &\textsf{map}_F\; \overline{id}^l\; x = x \tag{\textsc{map\_id}}\label{eq:map_id}\\
          &\textsf{map}_F\; \overline{g}^l\; (\textsf{map}_F\; \overline{f}^l\; x) =
            \textsf{map}_F\; \overline{(g \circ f)}^l\; x \tag{\textsc{map\_comp}}\label{eq:map_comp}\\
          &(\forall i.\; \forall z \in \textsf{set}_{F,i}\; x.\; f_i\; z = g_i\; z) \Longrightarrow 
            \textsf{map}_F\; \overline{f}^l\; x = \textsf{map}_F\; \overline{g}^l\; x \tag{\textsc{map\_cong}}\label{eq:map_cong}\\
          &\forall i.\; \textsf{set}_{F,i} (\textsf{map}_F\; \overline{f}^l\; x) = f_i\; \grave{\phantom{\_}}\; \textsf{set}_{F,i}\; x \tag{\textsc{set\_map}}\label{eq:set_map}\\
          &\textsf{infinite}\; \textsf{bd}_F \land \textsf{regular}\; \textsf{bd}_F \land 
            \textsf{cardinal\_order}\; \textsf{bd}_F \tag{\textsc{bd}}\label{eq:bd}\\
          &\forall i.\; | \textsf{set}_{F,i}\; x | <_o \textsf{bd}_F \tag{\textsc{set\_bd}}\label{eq:set_bd}\\
          &\textsf{rel}_F\; \overline{R}^l\; \bullet\; \textsf{rel}_F\; \overline{Q}^l\; = 
            \textsf{rel}_F\; \overline{(R\; \bullet\; Q)}^l \tag{\textsc{rel\_compp}}\label{eq:rel_compp}\\
          &\textsf{rel}_F\; \overline{R}^l\; x\; y = \tag{\textsc{in\_rel}}\label{eq:in_rel}\\
          &  \exists z.\; (\forall i.\; \textsf{set}_{F,i}\; z \subseteq \{(a, b).\; R_i\; a\; b \}) \land 
              \textsf{map}_F\; \overline{fst}^l\; z = x \land \textsf{map}_F\; \overline{snd}^l\; z = y
        \end{align*}
        \newline
        \footnotesize
      ...
      \end{figure}
      Additionally to the axioms we already motivated in \autoref{subsec:bnf_constants} (\autoref{eq:map_id} and \autoref{eq:map_comp} for the functoriality of $F$, \autoref{eq:set_map} to ensure that the setters are natural transformations and the boundedness of the setters \autoref{eq:set_bd}), we have four additional ones.

      One of those is the the congruency \autoref{eq:map_cong} of the map function. It states that if two (lists of) functions $\overline{f}^l$ and $\overline{g}^l$ are equal when applied to the corresponding sets of all components of an $F$ (obtained through the setters), then mapping these two lists of functions over the $F$ each produces the same result. When this property holds, we can be sure, that the mapper only depends on how the functions $\overline{f}^l$ behave on the components of the $F$ element.

      The axiom \autoref{eq:bd} just ensures that the bound $\textsf{bd}_F$ is a suitable cardinal, i.e., a regular and infinite one.

      Distributivity of the relator is formulated in \autoref{eq:in_rel}. We note here, that for showing that a type constructor is a \ac{BNF}, it is only necessary to prove the inclusion $(\textsf{rel}_F\; \overline{R}^l\; \bullet\; \textsf{rel}_F\; \overline{Q}^l)\; x\; y \Rightarrow \textsf{rel}_F\; \overline{(R\; \bullet\; Q)}^l\; x\; y $. The other direction follows automatically from this and the next axiom, weak pullback preservation.

      Lastly, weak pullback preservation \autoref{eq:in_rel} is the most abstract and complex axiom. The idea is that two elements $x$ and $y$ of the type $\alpha\; F$ are related through a relation $R$ iff there exists a $z$ that acts as a "zipped" version of $x$ and $y$. The components of this $z$ are $R_i$-related pairs of the components of $x$ and $y$, where the first position in the pair corresponds to $x$ and the second one to $y$.
    ...
    \subsection{Non-emptiness witnesses}
      ...
      \begin{equation*}
        \forall i.\; \textsf{set}_{F,i}\; (\textsf{wit}_F\; \dashedoverline{$w$}) = (\texttt{if}\; \textsf{wit}_F\; \text{depends on}\; \alpha_i\; \texttt{then}\; \{w_i\}\; \texttt{else}\; \emptyset)\tag{\textsc{wits}}\label{eq:wits}
      \end{equation*}
    ...
    \subsection{BNF examples}
      ...
      \begin{align*}
        ...
        \textsf{rel}_\textsf{plist}\; R\; Q &= \textsf{rel}_\textsf{list} (\textsf{rel}_\textsf{prod}\; R\; Q)
      \end{align*}
      \noindent where we use the standard map, set and relator functions of the list and product type.

      To show that \type{($\alpha$, $\beta$) plist} is a \ac{BNF}, we have to prove the \ac{BNF}-axioms for it. Besides the definitions above, we give $\aleph_0$ as the bound $\textsf{bd}_\textsf{plist}$.
  ...
  \section{Map-Restricted Bounded Natural Functors (MRBNFs)}
    ...
    The mapper and setters are expanded to work for the frees and bounds just as they do for lives. Thus, a \ac{MRBNF} $F$ with $l$ lives, $\fr$ frees and $b$ bounds has $l + \fr + b$ setters and a mapper with arity $l + \fr + b + 1$. Since the mapper takes small-support functions and bijections for the free and bound variables, which have the same type for their domain and range, this transfers to $F$ as well. This means that the type variables for frees and bounds are the same for the $F$ argument of the mapper and the result, while the lives can change type.

    We keep the original relator that only relates live variables with given relations and relates the free and bound variables to be with equality. Thinking in our model of $F$ elements being shapes with atoms in slots, the regular relator $\textsf{rel}_F$ requires the frees and bounds in each slot to be the same for both elements that are compared.
    
    To relate $F$ elements that are not equal in all frees and bounds, we introduce a new map-restricted relator $\textsf{mr\_rel}_F$. It takes a function for each free and bound - with the appropriate restrictions to small-support and bijectivity - in addition to the relations for the lives. The new arguments are placed in front of the relations for the live variables. It is then defined in terms of the relator as follows:
    \[
      \textsf{mr\_rel}_F\; \langle\overline{u}^{b}\; \overline{v}^{\fr}\rangle\; \overline{R}^l\; x\; y = \textsf{rel}_F\; \overline{R}^l\; x\; (\textsf{map}_F\; \langle\overline{id}^{l}\; \overline{v}^{\fr}\; \overline{u}^b\rangle\; y)
    \]
    where we write $f^l$ for the functions or relations of the live variables $f_1 \dots f_l$ and analogously $v^\fr$ and $u^b$ for frees and bounds. Furthermore, we write the arguments of the map function as $[\overline{f}^{l}\; \overline{v}^{\fr}\; \overline{u}^b]$. The argument order of the mapper might be different, as the lives, bounds and frees do not have to be separated, but can be interlaced. For example for the type $(\alpha,\; \beta,\; \gamma)\; F$ where $\alpha$ and $\gamma$ are free, while $\beta$ is bound, the mapper has the following type:
    \[
    ...
    \]
    Thus, in our notation $\textsf{map}_F\; \langle\overline{f}^{l}\; \overline{v}^{\fr}\; \overline{u}^b\rangle$ we assume the $\langle\dots\rangle$ to interlace the arguments correctly to $\textsf{map}_F\; v_1\; u_1\; v_2$. Analogously for the $\textsf{mr\_rel}_F$, $\langle\overline{u}^{b}\; \overline{v}^{\fr}\rangle$ interlaces the functions according to the order of bounds and frees.
    ...
    \subsection{MRBNF axioms}
      ...
      For the \autoref{eq:map_comp}, \autoref{eq:map_cong} and \autoref{eq:set_map} axioms, we add the assumptions that the functions that correspond to frees and bounds are small-support functions and that the ones corresponding to bounds are additionally bijections. 

      Furthermore, while \autoref{eq:rel_compp} stays unchanged, using the original relator, \autoref{eq:in_rel} is changed to be defined in terms of the map-restricted relator.
      % TODO copy changes
