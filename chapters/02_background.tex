...
% TODO cite Isabelle datatype pdf
  ...
  \section{Bounded Natural Functors (BNFs)}
    ...
    \subsection{Non-emptiness witnesses}
      ...
      \acs{BNF} carry non-emptines witnesses as proof that the type contains at least one element. Witnesses may depend on a subset of the \ac{BNF}'s live variables. For example a witness of $('a_1,\; \dots\; 'a_l)\; F$ that depends on the first and last type variable of $F$, this witness has the type $\textsf{wit}_F ::\; 'a_1\; \Rightarrow\; 'a_l\; \Rightarrow\; ('a_1,\; \dots\; 'a_l)\; F$. It denotes that given witnesses for the types $'a_1$ and $'a_l$, a witness for $F$ can be constructed. 
      
      Witnesses have to fulfill the following properties: For all type variables $'a_i$ the witness depends on, the witness may only contain the $'a_i$ elements $w_i$, that were given to the witness as arguments, i.e., $textsc{set}_{F,i}$ applied to the witness evaluates to the singleton $\{w_i\}$. Furthermore, the witness must not contain any elements of the live type variables $'a_j$, the witness does not depend on, i.e., $\textsc{set}_{F,j}$ must be empty. We formalize these properties in the following where \dashedoverline{$w$} denotes the arguments that the witness depends on.
      \begin{equation*}
        \textsc{wits: }\forall i.\; \textsf{set}_{F,i}\; (\textsf{wit}_F\; \dashedoverline{$w$}) = (\texttt{if}\; \textsf{wit}_F\; \text{depends on}\; 'a_i\; \texttt{then}\; \{w_i\}\; \texttt{else}\; \emptyset)
      %\forall i \in \{i.\; \textsf{wit}_F\; \text{depends on}\; 'a_i\}.\; \textsf{set}_{F,i}\; (\textsf{wit}_F\; \dashedoverline{$w$}) &= \{w_i\}\\
      %\forall i \in \{i.\; \textsf{wit}_F\; \text{does not depend on}\; 'a_i\}.\; \textsf{set}_{F,i}\; (\textsf{wit}_F\; \dashedoverline{$w$}) &= \emptyset
      \end{equation*}
  ...
  \section{Map-Restricted Bounded Natural Functors (MRBNFs)}
    ...
    \subsection{MRBNF axioms}
      TODO: changes to \autoref{fig:bnf_axioms}
