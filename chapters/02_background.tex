...
%    WIP: Considering a polymorphic type that is meant to represent simple $\lambda$-terms, where $\alpha$ is the space of variable names. If we want to substitute a free variable $x$ in a term $T$ by a term $N$, we may run into the following problem: If $T = \lambda y. T'$, we need to ensure that there are no name clashes with $y$ in the new term $N$ before we substitute $x$ by $N$ in $T'$. This is done by choosing a fresh $y'$ and renaming $y$ to $y'$ in $T'$. 
 ...
 \section{Map-Restricted Bounded Natural Functors (MRBNFs)}
   ...
   \label{subsec:binders}
     ...
     In general, any name occurring between \textbf{binds} and \textbf{in} corresponds to a type variable that is to be bound. In contrast to that, a name that occurs after the \textbf{in} corresponds to a recursive occurrence of the datatype in which the variables are being bound. Most importantly, the \textbf{binder\_datatype} command allows us to use an \ac{MRBNF} with bound variables in the definition of a datatype, if the bound variables correspond to type variables that are to be bound.

     The result of this command is a binding-aware datatype. This datatype considers $\alpha$-equivalent terms equal, i.e., it is quotiented by alpha equivalence, supports capture-free variable substitution, and features relevant for reasoning about lambda terms or other syntaxes with bindings. More details about the details on how these are given by Blanchette et al.~\mbox{\cite[§5]{blanchette2019bindings}}.
